...
def post_configuration_purge_pending(session, config_path):
    
    payload = {
        'node-path': config_path
    }

    post_url = 'configuration/object/configuration_purge_pending'

    if (session.api_verbose == True):
        print(f'Verbose: Sending POST to \'{session.api_url}{post_url}\' to purge pending configuration')
